...
Request.prototype.proxy = function( opts, callback ) {
  if( this.source !== 'network' || this.method !== 'INVITE' ) throw new Error('Request#proxy can only be used for incoming INVITES') ;
  ...
  opts.stackTxnId = this.stackTxnId ;
  opts.type = opts.type || 'stateless' ;

  this.agent.proxy( opts, callback ) ;
} ;
