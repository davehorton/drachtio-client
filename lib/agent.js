...
var onData = function(agent, msg) {
	...
	while( msg.length > 0 ) {
		...
		if( 0 === agent.incomingMsg.length ) {
			...
			agent.incomingMsg += msg ;
			pos = msg.indexOf('#') ;
			if( -1 === pos2 ) throw new Error('invalid message from server, did not start with length#: ' + msg) ; //cant split twice in a length fragment
			msg = parseMessageHeader( agent, msg, pos2) ;
		}
		else {
