...
Agent.prototype.proxy = function( req, opts, callback ) {
	var msg = 'proxy|' + opts.stackTxnId + '|' + opts.type + '|' + (opts.fullResponse ? 'fullResponse' : '') + '|' + 
		(opts.followRedirects ? 'followRedirects' : '') + '|' + opts.provisionalTimeout + '|' + opts.finalTimeout + '|' +
...
var proxy = function( opts, callback ) {
	var results = {} ;
	opts.destination.forEach( function( dest ){
		results[dest] = {
			status: 0,
			reason: '',
			timestamp: ''
	...
	return function( token ) {
		debug('TODO: proxy response handler, tokens: ', token) ;
