...
DigestClient.prototype.authenticate = function(callback) {
  ...
  debug('authenticate: options: ', options) ;
  ...
  if( typeof options.auth === 'function' ) {
    ...
    fn = function( callback ) { return callback( null, options.auth.username, options.auth.password ) ; } ;
  }
  else {
  ...
  fn( function(err, username, password) {
    if( err ) { 
      return callback( err ); 
    }

    debug(`going to try to auth with username ${username} password ${password}`); 
