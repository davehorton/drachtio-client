...
DigestClient.prototype.authenticate = function(callback) {
  ...
  if( typeof options.auth === 'function' ) {
    ...
    fn = function( res, callback ) { return callback( null, options.auth.username, options.auth.password ) ; } ;
  }
  else {
    callback(new Error('no credentials were supplied to reply to server authentication challenge')) ;
  }

  // note: we pass the 401 (or whatever) response in case the caller wants to see it
  fn( this.res, function(err, username, password) {
    if( err ) { 
      return callback( err ); 
    }
